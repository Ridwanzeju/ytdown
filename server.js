...
app.get('/api/youtube/download', async (req, res) => {
  ...
  try {
    ...
    if (format === 'mp3') {
      cmd = `python3 -m yt_dlp --no-playlist -f bestaudio --extract-audio --audio-format mp3 --audio-quality ${bitrate}K -g "${safeUrl}"`;
    } else {
      const heightMap = { '360': 360, '480': 480, '720': 720, '1080': 1080 };
      const h = heightMap[quality] || 720;
      cmd = `python3 -m yt_dlp --no-playlist -f "bestvideo[height<=${h}][ext=mp4]+bestaudio[ext=m4a]/best[height<=${h}][ext=mp4]/best" -g "${safeUrl}"`;
    }
