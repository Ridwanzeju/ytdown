...
app.get('/api/youtube/download', async (req, res) => {
  ...
  if (format === 'mp3') {
    ...
    // PROGRESSIVE FIRST: video+audio sudah jadi satu file dari YouTube,
    // jadi server TIDAK perlu mux pakai ffmpeg secara realtime -> jauh lebih cepat & ringan CPU.
    // Untuk 1080p, YouTube hampir selalu tidak punya progressive format,
    // jadi otomatis fallback ke merge (butuh ffmpeg, lebih lambat).
    args = [
      '-m', 'yt_dlp', '--no-playlist',
      '-f', `best[height<=${h}][ext=mp4]/bestvideo[height<=${h}][ext=mp4]+bestaudio[ext=m4a]/best[height<=${h}]/best`,
